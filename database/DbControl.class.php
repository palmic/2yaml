		...
		if (strtolower(self::$debug) == "firephp") {
			switch (true) {
				...
				case is_numeric(strpos($sql, "DELETE")):
						LBoxFirePHP::warn(" - ". str_replace("\n", "", self::$queryCount .": ". $sql));
					break;
				default:
						LBoxFirePHP::log(" - ". str_replace("\n", "", self::$queryCount .": ". $sql));
			}
		}
		else {
			switch (true) {
			...
			$color	= "#ffffff";
			$msg 	= "<table><th bgcolor='$bg' align='left'><font color='#C0C0C0'>". "</font></th><th bgcolor='$bg' align='left'><b><font color='$color'>". nl2br(self::$queryCount .": ". $sql) ."</font></b></th></table>\n";
			echo $msg;
			flush();
			}
		}
